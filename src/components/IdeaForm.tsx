...
export default function IdeaForm({ onSave, disabled = false }: IdeaFormProps) {
  ...
  return (
    <motion.div
      ...
      className={`w-full rounded-[24px] p-6 md:p-8 relative overflow-hidden border border-white/[0.12] bg-[#0c0715]/88 backdrop-blur-[24px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.75),0_0_50px_rgba(6,182,212,0.03),inset_0_1px_1px_rgba(255,255,255,0.02)] transition-opacity duration-300 ${disabled ? 'opacity-70' : ''}`}
    >
      ...
      <div className="mb-6 relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="font-display text-xl md:text-2xl font-extrabold text-white tracking-tight flex items-center gap-3">
            ...
            {disabled ? 'Idea Vault (Offline Mode)' : 'Capture Idea'}
          </h2>
          <p className="text-white/50 text-xs mt-1 font-light">
            {disabled ? 'Cannot add new ideas while offline. Syncing will resume when online.' : 'Turn a raw spark into a clear concept worth building.'}
          </p>
        </div>
        <div className="flex items-center gap-2 self-start md:self-center px-3 py-1.5 rounded-xl bg-cyan-500/8 border border-cyan-500/20 text-[10px] font-mono text-cyan-300 uppercase tracking-wider font-bold">
          ...
          <span>{disabled ? 'OFFLINE READ-ONLY MODE' : 'CYAN V2 CORE ACTIVE'}</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 relative z-10" id="new-idea-form">
        <div className="space-y-4">
          <div className="group relative">
            <input
              ...
              placeholder={disabled ? 'Adding ideas is disabled offline' : 'Give your breakthrough thought a name...'}
              className="w-full rounded-2xl glass-input px-4 py-3.5 text-sm text-white placeholder-white/25 focus:outline-none transition-all font-sans font-medium disabled:opacity-50 disabled:cursor-not-allowed"
              maxLength={100}
              disabled={disabled}
            />
          </div>
          ...
          <div>
            <textarea
              ...
              placeholder={disabled ? 'Adding ideas is disabled offline' : 'Describe your vision, technical specs, or design elements...'}
              rows={4}
              className="w-full resize-none rounded-2xl glass-input px-4 py-3.5 text-sm text-white placeholder-white/25 focus:outline-none transition-all font-sans leading-relaxed disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={disabled}
            />
          </div>
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-2.5 text-xs font-semibold rounded-xl border backdrop-blur-md transition-all duration-300 ease-out cursor-pointer text-center disabled:opacity-50 disabled:cursor-not-allowed ${
        ...
        <div className="flex justify-end pt-2">
          <motion.button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-gradient-to-r from-cyan-400 via-purple-500 to-indigo-600 text-white rounded-2xl font-bold transition-all duration-300 cursor-pointer text-sm font-sans disabled:from-gray-700 disabled:to-gray-800 disabled:text-white/40 disabled:cursor-not-allowed"
            disabled={disabled}
          >
            <Send className="h-4 w-4" />
            {disabled ? 'Offline Mode' : 'Save Idea'}
          </motion.button>
        </div>
      </form>
    </motion.div>
